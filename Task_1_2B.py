...
def detect_note_duration(audio_file):
	...
	no_of_frames=file_length/frame_size
	rms = sqrt(mean(square(sound)))
	...
	for i in range(0,file_length-frame_size):
		arr=sound[i:i+frame_size]
		rms1 = sqrt(mean(square(arr)))
		if rms1<threshold/5:
		...
		if flag0==flag1==True:
			flag0=False
			Onsets.append(round(float((1.00*i)/fs*1.0),2))
	

			threshold=rms
			j=j+1

		elif flag1==flag0==False:
			Silence.append(round(float((1.00*i)/fs*1.0),2))
			threshold=rms
			flag0=True
			k=k+1
	Silence.append(round(1.00*file_length/fs,2))
	
	
	for i in range(0,len(Onsets)):
		Note_durations.append([Onsets[i],Silence[i]])
...
def detect_silence_duration(audio_file):
	...
	for i in range(0,len(Onsets)-1):
		Silence_durations.append([Silence[i],Onsets[i+1]])
	
	if len(Silence)>len(Onsets):
		Silence_durations.append([Silence[len(Silence)-2],Silence[len(Silence)-1]])
