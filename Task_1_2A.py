...
def onset_detect(audio_file):
	...
	for i in range(0,no_of_frames):
		arr=sound[int(i*frame_size):int(i*frame_size+frame_size)]
		...
		if flag0==flag1:
			...
			Onsets.append(round(float((1.00*i*frame_size)/fs*1.0),2))
	...
	NewOnsets = Onsets[:]
	NewOnsets.append(file_length)
		
	for i in range(0,len(NewOnsets)-1):

		soundblock=sound[int((NewOnsets[i])*fs):int((NewOnsets[i+1])*fs)]
		...
		fs=44100
		x=soundblock[int(0.09*fs):int(0.09*fs+M)]
		frame_chunk=1
		while True:
		...
		#freq=ploc[arrr[len(arrr)-1]]*fs/N
